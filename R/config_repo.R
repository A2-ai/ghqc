...
#' @return this function is used for its effects, but will return if bool if the result is a config repo found locally at config_path
#'
...
check_ghqc_configuration <- function(config_path = ghqc_config_path()) {
  if (!interactive()) {
    cli::cli_abort(
      "Attempting to run in non-interactive function. Use {.code download_ghqc_configuration()} in non-interactive sections"
    )
  }
  check_ghqc_config_repo_exists()
  is_local <- switch(
    config_repo_status(config_path),
...
download_ghqc_configuration <- function(
  ...
  check_ghqc_config_repo_exists()
  is_local <- switch(
    config_repo_status(config_path, .force),
...
remove_ghqc_configuration <- function(config_path = ghqc_config_path()) {
  cli::cli_inform(
    "Removing downloaded custom configuration in {config_path}..."
  )
  tryCatch(
    {
      if (fs::dir_exists(config_path)) {
        fs::dir_delete(config_path)
      }
      cli::cli_alert_success(
        "Custom configuration in {config_path} successfully removed"
      )
    },
    error = function(e) {
      cli::cli_abort(
        "Custom configuration in {config_path} not removed due to {e$message}"
      )
    }
  )
  config_path
...
config_repo_status <- function(config_path, .force = FALSE) {
  ...
  if (remote_repo_updates(config_path)) {
    return("update")
  }
  return("none")
}
...
remote_repo_updates <- function(config_path) {
  remote_repo_updates <- gert::git_remote_ls(
    repo = config_path,
    verbose = FALSE
  )$oid[1] !=
    gert::git_info(repo = config_path)$commit
}

# repo clone #

#' @importFrom cli cli_alert_danger
#' @return bool if the result is a config repo found locally at config_path
prompt_repo_clone <- function(config_path) {
  cli::cli_alert_danger(sprintf(
    "Custom configuration repository %s is not found locally",
  ...
  if (yN == "y") {
    ...
    cli::cli_alert_danger(
      "Run {.code ghqc::download_ghqc_configuration()} before running any ghqc ecosystem apps"
    )
    FALSE
  }
}
...
prompt_repo_update <- function(config_path) {
  cli::cli_alert_warning(sprintf(
    "Custom configuration repository %s was found locally, but is not the most recent version",
    basename(config_path)
  ))
  yN <- readline(
    prompt = glue::glue(
      "Would you like to update the repository? This will delete all local changes to {config_path} (y/N) "
    )
  )
  if (yN == "y") {
    repo_clone(config_path)
...
repo_clone <- function(config_path) {
  if (!rlang::is_installed("gert", version = "1.5.0")) {
    cli::cli_alert_danger(sprintf(
      "Package 'gert' (>= 1.5.0) is not installed. %s cannot be cloned",
      config_repo_url()
    ))
    return(FALSE)
  }
  ...
  tryCatch(
    {
      ...
      cli::cli_alert_success(
        "Successfully cloned {config_repo_name()} to {config_path}"
      )
    ...
    error = function(e) {
      cli::cli_abort(
        message = c(
          sprintf("Clone of %s was not successful", config_repo_name()),
...
no_updates <- function(config_path) {
  invalidate_checklists(config_path)
  cli::cli_inform(
    "Custom Configuration Repository found up to date at {config_path}"
  )
  config_files_desc(config_path)
  TRUE
}
...
no_gert_found <- function(config_path) {
  ...
  cli::cli_alert_warning(
    "Package 'gert' (>= 1.5.0) was not installed to check if custom configuration repository is up to date"
  )
  TRUE
}
...
config_files_desc <- function(config_path) {
  ...
  if (fs::file_exists(repo_files[1])) {
    cli::cli_alert_success(paste0(
      cli::col_blue("logo.png"),
      " successfully found"
    ))
  } else {
    cli::cli_alert_info(paste0(
      cli::col_blue("logo.png"),
      " not found. This file is not required."
    ))
  }
  cli::cli_inform(" ")
...
custom_options_found <- function(yaml_path) {
  ...
  if (is.null(content)) {
    cli::cli_alert_danger(paste0(
      cli::col_blue("{basename(yaml_path)}"),
      " could not be read"
    ))
    return()
  }
  ...
  if (
    all(
      !(c("prepended_checklist_note", "checklist_display_name_var") %in%
        names(content))
    )
  ) {
    cli::cli_alert_warning(paste0(
      "No recognized custom options found in ",
      cli::col_blue("{basename(yaml_path)}")
    ))
    return()
  }
  ...
  ul <- cli::cli_ul()
  sapply(names(content), function(x) {
    switch(
      x,
...
check_ghqc_config_repo_exists <- function() {
  ...
  if (substr(config_repo, 1, 8) != "https://") {
    cli::cli_abort(
      "GHQC_CONFIG_REPO ({config_repo}) does not start with 'https://'"
    )
  }
}
...
config_repo_not_found <- function() {
  cli::cli_abort(
    message = "GHQC_CONFIG_REPO not found. Please set in ~/.Renviron"
  )
}
