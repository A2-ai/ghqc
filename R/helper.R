...
error_checks <- function(app_name, qc_dir, lib_path, config_path) {
  if (!fs::dir_exists(qc_dir)) {
    cli::cli_abort(paste(qc_dir, "does not exist."))
  }
  if (!fs::dir_exists(lib_path)) {
    cli::cli_abort(paste(
      lib_path,
      "does not exist. Refer to installation guide and check library path is set to correct location."
    ))
  }
  if (
    !(app_name) %in%
  ...
  if (!fs::dir_exists(config_path)) {
    cli::cli_abort(c(
      ...
      "Run {.code ghqc::check_ghqc_configuration(config_path = '{config_path}')} to verify proper setup."
    ))
  }
}
  ...
  tryCatch(
    {
      if (is.null(ghqcapp_pkg_status(lib_path))) {
        rlang::abort(
          message = glue::glue(
            "ghqc.app not installed in {lib_path}. Please install before running any ghqc apps"
          )
        )
      }
      ...
      new_rstudioapi <- compareVersion(
        utils::packageDescription("rstudioapi", fields = "Version"),
        "0.16.0"
      ) !=
        -1
        ...
        if (new_rstudioapi) {
          if (rstudioapi::jobGetState(job_id) == "failed") {
            ...
            cli::cli_alert_danger(
              "Shiny app could not be started due to error (see Background Jobs panel)"
            )
            break
          }
        }
      ...
      if (counter > iterations) {
        ...
        if (new_rstudioapi) {
          ...
          # else, was either a timeout or error
          cli::cli_alert_danger(
            "Shiny app could not be started due to timeout or error"
          )
        }
      }
