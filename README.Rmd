...
- **Logo**: A logo to be included in the header of the resultant record's header named as "logo.png"
...
In addition to downloading the repository, or remote updates if the repository was previously downloaded, the structure of the repository and the checklist yamls are verified.
...
- INSTALL PACKAGES
  ...
  - The dependency packages are installed by default to "~/.local/share/ghqc/rpkgs". To install to the default location, which is highly recommended, hit `ENTER`. Otherwise, type in the path in which to install the dependency packages to.
